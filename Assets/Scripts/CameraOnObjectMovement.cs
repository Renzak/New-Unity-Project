...
public class CameraOnObjectMovement : MonoBehaviour
{
    ...
    public float maxCameraHeight = 0.4f;
    public float minCameraHeight = 0.1f;
    ...
    {
        oldX = 0f;
        oldY = Mathf.Acos((minCameraHeight + maxCameraHeight) / 2f);
    ...
    void UpdateCameraPosition(float mouseX, float mouseY)
    {
        ...
        if (Mathf.Cos(mouseY + oldY) > maxCameraHeight || Mathf.Cos(mouseY + oldY) < minCameraHeight)
        {
            mouseY = 0;
        }
